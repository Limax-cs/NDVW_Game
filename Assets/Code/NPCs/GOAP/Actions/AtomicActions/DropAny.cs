...
public class DropAny : GAction
{
    ...
    {
        foreach(GameObject g in agentData.backpack)
        {
            if (g is not null)
                return true;
        }
    ...
    public override void Perform()
    {
        if (!droppedItem)
        {
            ...
            {
                if (agentData.backpack[k] is not null)
                {
                    float itemValue = 1;
                    if (itemValue < value)
                    {
            ...
            // Drop Item
            agentData.backpack[backpackIndex].transform.SetParent(null);
            agentData.backpack[backpackIndex] = null;
            droppedItem = true;
        }
    }
