...
public class GoToX : GAction
{
    ...
    public void CalculateReachability(Vector3 originPos)
    {
        ...
        //Debug.Log("Target: " + target.name + " | Position Agent: " + transform.position + " | Target position: " + target.transform.position + " | has path: " + hasPath);
    ...
    public override bool IsFinished()
    {
        ...
        if (isReachable && running)
        {
            ...
            //        " | Agent HasPath: " + agentData.agent.hasPath + 
            //        " | Agent Distance: " + agentData.agent.remainingDistance + 
            //        " | Agent Distance 2: " + Vector3.Distance(target.transform.position, agentData.transform.position) + 
            //        " | Belief Trigger: " + agentData.beliefs.HasState(this.beliefTrigger));

            float targetDist = 0;
            if (target != null)
            {
                targetDist = Vector3.Distance(target.transform.position, agentData.transform.position);
            }

            if ((agentData.agent.hasPath && agentData.agent.remainingDistance < distance && targetDist < distance + 2.0f)||(agentData.beliefs.HasState(this.beliefTrigger)))
            {
                //Debug.Log("Action Name: "+ this.actionName +
                //    " | Is Reachable: " + isReachable + 
                //    " | Agent HasPath: " + agentData.agent.hasPath + 
